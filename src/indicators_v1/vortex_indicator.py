...
# Vortex Indicator (VI)
def calculate_vortex_indicator(df: pd.DataFrame, timeperiod: int):
    """
    Vortex Indicator (VI)
    ...
    df['Sum_VM+'] = df['VM+'].rolling(window=timeperiod).sum()
    df['Sum_VM-'] = df['VM-'].rolling(window=timeperiod).sum()
    df['TR'] = talib.TRANGE(df['high'], df['low'], df['close'])
    df['Sum_TR'] = df['TR'].rolling(window=timeperiod).sum()
